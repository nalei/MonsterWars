...
class FiringComponent: GKComponent {
  ...
  override func update(deltaTime seconds: TimeInterval) {
    ...
    guard let playerComponent = entity?.component(ofType: PlayerComponent.self), let spriteComponent = entity?.component(ofType: SpriteComponent.self) else {
      return
    }
    ...
    for enemyEntity in enemyEntities {
      ...
      if (CGFloat(abs(distance)) <= range + wiggleRoom && CGFloat(CACurrentMediaTime() - lastDamageTime) > damageRate) {
        ...
        let laser = Laser(player: playerComponent.player, damage: damage, entityManager: entityManager)
